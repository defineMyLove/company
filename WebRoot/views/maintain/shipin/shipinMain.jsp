...
<head>
    ...
    <script type="text/javascript">
        ...
        seajs.use(['$', 'pupZtree', 'ligerui'], function ($, pupZtree) {
            ...
            $("#usergrid").ligerGrid({
                ...
                columns: [
                    ...
                    { display: '路径', name: 'file_path', align: 'left'}
                ],
                url: '${path}/maintain/shipin/list',
                root: 'rows',
                record: 'total',
                toolbar: { items: [
        ...
        function itemclick(item) {
            ...
            if ('add' === item.id) {
                window.top.openDialog("添加功能","${path}/maintain/shipin/addUI",80,80, [
                ...
                window.top.openDialog("添加功能","${path}/maintain/shipin/addUI?id="+rows.id,80,80, [
                    { text: '确定', onclick: function (item, dialog) {window.top.submitForm()},cls:'l-dialog-btn-highlight' },
                    { text: '取消', onclick: function (item, dialog) { window.top.closeDialog(); }}
                ]);
                }else if ('del' === item.id) {
                ...
                $.ligerDialog.confirm("确定删除此数据么？",function(yes){
                    if(yes){
                        $.getJSON("${path}/maintain/shipin/del", {id: rows.id}, function (data) {
                ...
                window.top.openDialog("详情","${path}/maintain/shipin/detail?id="+rows.id,80,80, [
                    { text: '取消', onclick: function (item, dialog) { window.top.closeDialog(); }}
                ]);
            }
        }
